...
function plain(spans: RichText): string {
  return spans.map((span) => span.t === "cite" ? `[${span.n}]`
    : span.t === "strong" ? `**${span.s}**`
    : span.t === "em" ? `*${span.s}*` : span.s).join("");
}

/** Explicit user download; never writes AI output into the official wiki. */
export function answerMarkdown(question: string, answer: Answer, savedAt = new Date(), options: { includeSnapshot?: boolean } = {}): string {
  ...
  const lines = [
    ...
    ...answer.calc.flatMap((step) => [plain(step.text), ""]),
  ];
  if (answer.sensitivity) lines.push(`## ${answer.sensitivity.title.replace(/^(?:◭\s*)?敏感性(?:\s*[·:：]\s*)?/, "").trim() || "补充说明"}`, "", plain(answer.sensitivity.text), "");
  lines.push("## 引用", "");
  ...
  if (options.includeSnapshot === false) return lines.join("\n");
