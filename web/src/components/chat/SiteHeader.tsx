...
const NAV_ITEMS = [
  { label: "聊天", href: "#", active: true },
  { label: "模拟器", href: "#", active: false },
  { label: "军表实验室", href: "#", active: false },
  { label: "图鉴", href: "#", active: false },
];
      ...
      <nav className="mx-auto flex max-w-[1100px] gap-1 px-5 max-tablet:scrollbar-none max-tablet:overflow-x-auto max-tablet:px-2">
        {NAV_ITEMS.map((item) => (
          <a
            key={item.label}
            href={item.href}
            className={`clip-slant-8 pt-2 pb-[9px] font-cond text-[14px] tracking-[2px] uppercase no-underline max-tablet:px-3.5 max-tablet:text-[13px] max-tablet:tracking-[1px] max-tablet:whitespace-nowrap ${
              item.active
                ? "bg-[linear-gradient(var(--color-gw-red),#6e0505)] px-[22px] text-white [text-shadow:0_1px_2px_#000]"
                : "bg-[#16211f] px-[22px] text-[#97a4a0] hover:bg-[#1d2b28] hover:text-bone"
            }`}
          >
            {item.label}
          </a>
