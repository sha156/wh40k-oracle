...
class TestMaterialize:
    ...
    def test_real_payload_projection_counts(self, tmp_path):
        ...
        # + 142（兽人 PR14）+ 83（圣血修女 PR15）+ 98（灰骑士 PR16）= 1279
        assert rep["applied"] + rep["already"] == len(all_entries) == 1279
        assert not rep["fingerprint_mismatch"] and not rep["skipped"]
        assert rep["by_status"] == {"encoded": 103, "partial": 269, "not_modeled": 907}
