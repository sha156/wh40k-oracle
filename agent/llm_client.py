...
_TOOL_ARG_HINTS: Dict[str, str] = {
    ...
    "judge_fight_order": "{}",
    "simulate_combat": '{"attacker": "A", "defender": "B"}',
...
工具使用策略：
- 查某个单位/实体（数据、武器、技能）时，**直接用用户原文里的中文单位名调 get_entity**，
...
- get_entity/search_wiki 查空时，再退而用 rag_search 做自然语言兜底检索。
        ...
        *,
        ...
        if want_json:
            try:
                ...
                # 个别供应商/版本不支持 response_format，退回普通模式
