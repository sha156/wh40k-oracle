...
4. 数值权威源 = **社区结构化数据（Wahapedia CSV / BSData XML）**，英文 Faction Pack PDF
   降为交叉校验，中文版做术语对照和描述文本（2026-07-04 调研后调整，见第四节数据源策略）
...
7. wiki 兼容 **Obsidian vault**（2026-07-05 新增）：`wiki/` 可直接作为 Obsidian 库打开，
   人读走 Obsidian（标签面板/反链/关系图谱），AI 读走 Agent 工具，同一份文件两个入口
8. 最终交付形态 = **网站**（2026-07-05 新增）：Streamlit 是开发期本地壳，
   终态为 FastAPI 后端 + Next.js 前端（用户主栈），所有能力通过自己的网站对外提供
...
tags: [unit/tau-empire, battleline]        ← Obsidian 标签面板用，由 type/faction/keywords 生成
...
### Obsidian 兼容（wiki 即 vault）

`wiki/` 目录直接用 Obsidian 打开，零额外构建。人读与 AI 读共用同一份文件：

| Obsidian 能力 | wiki 侧约定 |
|---------------|------------|
| 双链/反链面板 | 正文交叉链接本来就是 `[[...]]` 语法，Obsidian 按文件名解析，路径式链接同样支持 |
| 标签面板 | frontmatter 增加 `tags:` 字段：`[unit/tau-empire, battleline]` 之类，由 type/faction/keywords 自动生成（标签不含空格，用 slug） |
| 别名搜索 | frontmatter 的 `aliases:` 字段与 Obsidian 原生别名约定同名，社区译名直接可搜 |
| 关系图谱 | 实体页互链后，图谱视图免费呈现"规则族关联"全景（总规则↔单位↔分队↔CP技能） |
| Dataview（可选插件） | frontmatter 的 points/faction/type 均为结构化字段，可写查询表（如"钛帝国所有 Battleline 单位点数表"） |
| index.md | 兼作 MOC（Map of Content）页 |

约定：编译器产出的 frontmatter 字段名以 Obsidian 原生约定优先（aliases/tags）；
`.obsidian/` 配置目录加入 .gitignore（个人视图配置不入库）。
...
模拟、算分、验表的地基。**永不手写数据**——发现错误改上游数据源或解析规则，重编译。

### 数据源策略（2026-07-04 调研更新）

调研发现英文数值数据**不必从 PDF 表格里抠**，社区已有两个机器可读权威源：

| 数据源 | 内容 | 用途 | 状态 |
|--------|------|------|------|
| **Wahapedia CSV 导出** | ~20张关系表：datasheets/models属性/costs点数/wargear/stratagems/enhancements/detachments/leader依附 | **L3 主数据源**（表结构几乎可直接映射 wh40k.sqlite） | 官方导出接口，随补丁更新 |
| **BSData/wh40k-10e** | 全阵营 BattleScribe XML（.cat）：属性/点数/武器/技能文本 + **组军约束**（min/max/Rule of 3） | 交叉校验 + **验表规则来源**（roster_validator 直接吃它的 constraints） | 255★，当天仍在更新 |
| 英文 Faction Pack PDF（`data_refined/`） | 页级文本 | 降为交叉校验与引用溯源 | 已完成 refine |
| 中文 codex PDF（`data_refined/`） | 中文术语/技能描述/规则正文 | wiki 正文与双语术语的唯一来源（社区源无中文） | 已完成 refine |

参考实现：`fjlaubscher/depot`（开源 Wahapedia companion app）的 CLI 已写好
CSV→JSON 的完整管线和 TypeScript 类型定义，字段 schema 直接照抄；
`pwestling/battlescribe-roster-parser` 可参考 .rosz（zip+xml）军表文件解析。

由此带来的简化：中英配对（wiki_compile ②）不再是"两堆 PDF 实体互猜"，
而是"中文 PDF 实体 → 结构化 DB 的 canonical 英文名"单向映射，配对锚点更稳；
武器词条在 Wahapedia 里已是独立字段，DSL 自动解析（录入策略第1条）覆盖面更大。
...
- 阶段漏斗：命中→造伤→穿保→最终伤害的逐阶段成功率（定位"伤害死在哪一步"，参考 wathammer.com）
- 性价比指标：每100点投入的期望伤害与击杀分值（点数从 SQLite 自动带入，无需手填）
- 反向视角：对方反打的同一套数字（"值不值得冲"要看交换比）
- **诚实声明**：本次模拟计入的技能清单 + `not_modeled` 未计入清单
- 敏感性提示：如"若对方使用CP技能盔甲坚守，期望击杀降至X"
- 多组模式：多武器组/多单位集火同一目标；多套配置方案并排对比（A/B loadout）
...
- 随机种子固定，结果可复现
- 性能：numpy 向量化掷骰（一次生成 N次迭代×攻击数 的骰池），万次迭代亚秒级，无需上编译语言
...
## 八、L6 UI（两阶段：本地 Streamlit → 网站）

**解耦原则（两阶段共用）**：L4 引擎 + L5 Agent 是纯 Python 库，不 import 任何 UI 框架；
UI 只是壳。这保证 Streamlit → 网站迁移时业务逻辑零改动。

### 阶段一：本地 Streamlit（开发期）

```
💬 聊天        主入口，全部能力可达；模拟类回答附"在模拟器中打开"按钮（带参跳转）
⚔️ 模拟器      下拉选攻/守单位→武器配置→增益开关（CP技能/光环/掩体）→分布图+阶段漏斗；
              支持多组集火与 A/B 方案对比
📋 军表实验室  粘贴军表→合法性报告+火力雷达图+点评；可增删单位实时重算
📖 图鉴        wiki 浏览器：阵营→单位/技能/分队，双语切换，交叉链接可点
              （本地人读优先走 Obsidian，此页签服务网站阶段）
```
...
```
FastAPI 后端（api/）
  POST /chat        Agent 对话（SSE 流式输出）
  POST /simulate    模拟引擎（SimContext 入参 → SimReport）
  POST /roster      验表/点评
  GET  /wiki/...    实体页只读渲染（terms/图鉴数据）
    ↑ 全部是 L4/L5 库的薄封装
Next.js + Tailwind 前端（用户主栈）：聊天/模拟器/军表/图鉴 四页面对位 Streamlit 页签
Streamlit 保留为本地调试壳，不下线
```

网站化约束：
- API key（DEEPSEEK 等）只存服务端环境变量；接口加限流与 CORS 白名单
- 网站只输出回答、模拟数字与引用页码，不提供规则书原文/数据库整体下载（版权边界）
- 会话上下文从 Streamlit session_state 平移为服务端 session（内存起步，不引数据库）
...
P1  wiki_compile ③-⑥ 全流水线，试点编译 钛帝国+吞世者 两阵营 + 核心规则
    └ 收益：图鉴页签可用，实体页质量肉眼可验
P2  db_compile：Wahapedia CSV 导入建 SQLite + BSData 约束导入 + calc_points + 实体解析器
    └ 收益：数据类问题从"检索碰运气"变精确查询；点数/属性天然随补丁更新
...
P8  网站化：FastAPI 封装 L4/L5 → Next.js+Tailwind 前端 → 部署上线
    └ 收益：通过自己的网站对外提供全部能力（最终交付形态）
```

Obsidian 无对应期数：P1 首批实体页落地当天，`wiki/` 就能用 Obsidian 打开
（tags/aliases 是编译器顺手写的 frontmatter 字段，零额外成本）。

依赖关系：P0→P1→P2→{P3, P4} 并行 → P5 → P6 → P8；P7 与 P5-P8 并行滚动。
P3 之后每一期都是独立增量，可按兴趣挑着做（ADHD 友好：每期 2-5 天粒度，有即时正反馈）。
