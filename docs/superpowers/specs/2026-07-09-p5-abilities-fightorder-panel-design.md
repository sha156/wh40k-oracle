...
```
Fight phase 分两步依次结算：
  Step 1「Fights First」：具备下列任一的**fights-first 来源**的单位先打——
       · 本回合冲锋成功的单位（charged this turn；注意冲锋只发生在当前玩家的冲锋阶段，
         故"charged"必然 == active player 一侧）
       · 拥有 Fights First 能力的单位
     若两边都有资格，从【当前玩家（active player）】的单位开始交替选取结算。
  Step 2「Remaining Combats」：其余单位，同样从 active player 开始交替选取结算。
  例外「Fights Last」的【抵消规则】（评审 E2 订正）：
       · 单位 fights_last 且【无任何 fights-first 来源】（未冲锋且无 Fights First）→ 押到全场最后；
       · 单位【同时】有 fights-first 来源 且 fights_last → 两者**相互抵消**，回到 Step 2 正常时序
         （既不先打也不押最后）。禁止无条件把 fights_last 降到队尾。
  打断「Counter-offensive」战略（2CP，评审 N2 订正）：在【某敌方单位刚结算完毕后】，
     玩家可选自己一个尚未结算的合格单位插队立刻结算（措辞上不硬限"非当前玩家"，实务多为对手用）。
```
...
判定分层（含 E2 抵消）：`has_ff = charged or fights_first`。分层键 =
`step1` 若 `has_ff and not fights_last`；`last` 若 `fights_last and not has_ff`；否则 `step2`（含 has_ff+fights_last 抵消回正常时序）。同层从 active player 起交替。`counter_offensive_by` 给定时，模拟该单位插队（在其后一位敌方单位结算后）并在 note 里说明差异。**每条分支一组单测**：active 方冲锋 vs 守方 Fights First（同入 step1，active 先）、fights_first+fights_last 抵消回 step2、纯 fights_last 押最后、charged+fights_last 抵消回正常时序、counter-offensive 打断、都不冲锋（active 先）。
...
4. **手工核验通用效果**（≤5 条，behind 显式 options 开关，每条 spec 附手算）：
   - `go_to_ground`（Infantry 单场景开关，1CP 战略，**评审 E1 订正**）→ **Benefit of Cover（射击护甲 +1，且 Sv≤3+ 对 AP0 不享受——沿用 P4 `effective_save`）+ 6+ 无效保护 Effect**。**不含 Stealth / 命中 -1**（那是 Smokescreen，别混）。
   - `smokescreen`（若建模）→ 掩体 Effect + Stealth(-1 射击命中) Effect（Cover+Stealth 组合是它，不是 Go to Ground）。
   - 其余一律 surface，不编码。
   - **FNP 非叠加铁律（评审 N3）**：即便一个单位检测到多条 FNP，引擎每点伤害**只掷一次** FNP、取更优阈值，**绝不累加/多次掷**（P4 `allocate_numpy` 已是单一 `fnp_thresh` 通道，天然满足；toggle 施加时只并入一个 fnp Effect，多条取最优 X）。
