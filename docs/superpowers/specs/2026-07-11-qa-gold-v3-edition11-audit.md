...
- **run2 = 93.8**（90✅/2⚠️/4❌）：#86/#88 转绿✅。剩余 4 ❌ 两轮一致，全是实体解析
  ...
  基础型），非版本问题。
- **实体缺陷修复（2026-07-11 同日）**：community 别名 4 条（撞名单位经 canonical id
  直取——Cultist Mob 三行/Skitarii Rangers 两行）+ 修 `_TOOL_ARG_HINTS` 截短指令
  （#65 真凶是提示词教 LLM 去掉"阵营前缀"，把「机械教游侠」截成「游侠」精确命中灵族
  Rangers）。run3 96.9 → **run4 = 99.0（95✅/1⚠️/0❌，零硬错）**，唯一 ⚠️ 为 #42
  题面歧义风格噪声。**v3 现行基线 = 99.0**，全部产物与逐轮核因归档
