...
> - ✅ 实体解析四缺陷修复（2026-07-11）：community 别名 4 条（撞名单位 canonical id 直取）
>   + `_TOOL_ARG_HINTS` 截短指令订正（#65 真凶）；**v3 基线 91.7→93.8→96.9→99.0 零硬错**，
>   逐轮核因见 benchmarks/v3_edition11/README；真库回归测试防重建丢失
> - ✅ S6 收官（2026-07-11）：core-rules 其余 53 个术语页全部 11 版化（64/64 完成）——
