...
- The Codex checkout at `C:/Users/Administrator/Documents/ChatGPT/RAG` will receive the verified implementation; local models and source caches remain local.
...
- [ ] Build the current frontend; test API, UI, retrieval and representative live LLM flows.
- [ ] Run full tests and an actual Docker build/start where host support permits; document concrete blockers.
- [ ] Review final diff; produce source-linked sync and acceptance reports; consolidate the verified checkout.
