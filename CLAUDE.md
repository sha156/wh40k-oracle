...
The user resumed work and then retired `data/沃坦联盟CODEX-双子星版 V1.30.pdf`. Its PDF/refined cache are in an excluded archive; 73 active chunks were removed (5,754 → 5,681), with every retained document/vector unchanged and SQLite byte-identical. Persistent policy guards block re-ingestion through PDF and derived-cache entry points. Full native suite **2,687 passed**; frontend **22**, type/lint and real Chrome **12/12** passed, including actual download/clipboard delivery. Docker is running with all assets and completed warmup; 117 deployed files match. Historical Calgar identity is repaired, but one of three latest comparisons still uses a lossless formatting fallback. Black Library capture remains partial (98 unresolved). **Removal of all other fan-translated PDFs is awaiting the user's scope answer; only the named PDF is retired.** See [current acceptance](docs/superpowers/reports/2026-09-26-local-cleanup-acceptance.md). Earlier paused headings below are historical.


## Latest paused Docker acceptance — 2026-09-26

Resume from `docs/superpowers/reports/2026-09-26-paused-final-docker-checkpoint.md` and the final Docker acceptance report. Reviewed evidence-retention and narrow answer-grounding fixes are deployed. Full native suite: **2,662 passed**; focused grounding: 112; corrected browser suite: 9/9 on the preceding API candidate. Current API `23855c9f...` matches all 111 checked Python files; assets/warmup pass. Seven fresh chats are **mixed 6/7**: a comparison falsely says the historical cache does not distinguish ordinary/armour Calgar. Diagnosis: `calc_points` omits the already-verified ordinary-card composition from its archived summary. The narrow projection fix is **not implemented** because the coordinated source task received the user's save-and-stop request. No crawl is needed for this defect. Docker remains running; source remains uncommitted, PR #75 was not updated/merged, and official points remain the September 14 snapshot. All older paused states below are historical where superseded here.

## Paused final-image regression fix — 2026-09-26

Docker acceptance details are saved in `docs/superpowers/reports/2026-09-26-paused-docker-checkpoint.md`: 2,646 native tests passed before the final draft; 22 frontend tests/type/lint and real roster/pricing checks passed. Three individual identity chats passed; the comparison failed. Existing browser suite passed 7/8 with an obsolete invalid-model expectation. The draft replacement E2E still intercepts `/simulate/combat` instead of the actual `/simulate` route and needs correction/review on resume. API image `c8841e4eaa8c376c8e6cd502b5738707a7aa4cdc802b756f9c975b45f55d6201` and web image `5156e1f53e6792b1f3de10da01106b885c5ba36e790c2a50cdaa48b2997cedda` remain running; latest loop fix is not deployed. No application/data work continued after the stop request.

User requested save and stop in the coordinated Docker task. See `docs/superpowers/reports/2026-09-26-paused-evidence-preservation-fix.md`. A final-image comparison reproduced a loop bug: a later empty alias lookup discarded earlier historical Calgar 200 and current Guilliman 355 evidence through automatic RAG fallback. A narrow source fix and paired regression are saved in `agent/loop.py` and `tests/test_agent_loop.py`; the positive test failed before and passed after, and the focused suite passed 56. `web/e2e/simulator-errors.spec.ts` now expects the reviewed client-side models=200 rejection and separately preserves backend-error rendering coverage. TypeScript, lint and diff-check passed. Independent review and E2E were interrupted/not run on stop; the fix is not rebuilt or deployed. Preserve the dirty tree, keep the separate crawl stopped, and follow the checkpoint resume order. The September 22 pause below is historical where superseded here.

## Paused review and source capture — 2026-09-22

User requested save and stop. Resume from `docs/superpowers/reports/2026-09-22-paused-review-crawl-checkpoint.md` and the coordinated Docker acceptance report. Historical Calgar archive lookup/provenance, rebuild preservation, malformed-source handling, roster validation/pricing, simulator composition counts and frontend stale-response fixes are implemented and reviewed. Native suite passed 2,628 before the final focused additions; final focused suites passed 133 and 69, frontend 22 plus type/lint, downloader 17. Latest web image is deployed; final API acceptance is paused. A separate Black Library snapshot saved all 1,171 inventory records and 1,071 full details across 38 faction/subfaction names, plus 2,514 powers and catalogues. It remains partial: 98 unresolved detail records and two known empty army-rule responses await final-script resume. Raw/output hashes match; canonical caches and official prices were not replaced. Source changes remain uncommitted on `codex/review-answer-provenance`; draft PR #75 remains unmerged. Earlier September 21 unfinished Calgar claims below are historical.

## Paused chat and Calgar continuation — 2026-09-21

User requested save and stop. See `docs/superpowers/reports/2026-09-21-paused-chat-calgar-checkpoint.md` for the exact saved state and resume order. Runtime remains `D:/Project/py/RAG`, branch `codex/review-answer-provenance`, draft PR #75 unmerged. Guilliman alias/lookup, evidence retention, simulator input validation and explicit DeepSeek Flash are implemented. The running chat restores the original Warhammer theme with simplified continuous dialogue and readable copy. Final style/readability revisions remain uncommitted. Ordinary Calgar exists in the Black Library cache as a deleted historical 200-point record; only the current Armour of Antilochus variant (155 in the September 14 official snapshot) exists in canonical units. A separate archive projector and restore hook are written (37 focused tests passed), but tool/formatter integration, runtime DB projection and deployment are unfinished. The integration test draft is currently failing and includes fixture problems. Do not claim Calgar lookup is fixed or the final tree fully tested. Local files are saved; Docker was left running. No further push/merge/deployment was performed after the stop request.
