---
id: 000003948
name_zh: 卡迪安突击队
name_en: Cadian Shock Troops
faction: 基因窃取者教派
type: unit
points:
  10 models: 65
  20 models: 120
tags:
- unit
- unit/基因窃取者教派
- 基因窃取者教派
version:
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 卡迪安突击队 | 6" | 3 | 5+ | 1 | 7+ | 2 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 激光手枪 | 24" | 1 | 4+ | 3 | 0 | 1 | [[core-rules/pistol.md\|手枪]] |
| 榴弹发射器（穿甲） | 24" | 1 | 4+ | 9 | -2 | D3 | — |
| 榴弹发射器（破片） | 24" | D3 | 4+ | 4 | 0 | 1 | [[core-rules/blast.md\|爆炸]] |
| 火焰喷射器 | 12" | D6 | N/A | 4 | 0 | 1 | [[core-rules/ignores-cover.md\|无视掩体]]，[[core-rules/torrent.md\|洪流]] |
| 爆弹手枪 | 12" | 1 | 4+ | 4 | 0 | 1 | [[core-rules/pistol.md\|手枪]] |
| 激光手枪 | 12" | 1 | 4+ | 3 | 0 | 1 | [[core-rules/pistol.md\|手枪]] |
| 热熔枪 | 12" | 1 | 4+ | 9 | -4 | D6 | [[core-rules/melta.md\|热熔2]] |
| 等离子枪（标准） | 24" | 1 | 4+ | 7 | -2 | 1 | [[core-rules/rapid-fire.md\|速射1]] |
| 等离子枪（过载） | 24" | 1 | 4+ | 8 | -3 | 2 | [[core-rules/rapid-fire.md\|速射1]]，[[core-rules/hazardous.md\|危险]] |
| 士官自动枪 | 24" | 2 | 4+ | 3 | 0 | 1 | — |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 链锯剑 | 近战 | 3 | 4+ | 3 | 0 | 1 | — |
| 格斗武器 | 近战 | 1 | 4+ | 3 | 0 | 1 | — |

## 技能
- **突击兵**：你的指挥阶段结束时，如果本单位处于你占领的目标点范围内，则这个目标点即使范围内没有你的模型，也算被你占领， 持续到任意回合开始时或结束时这个任务点被敌方占领
- **步话机**：当持有者所在单位作为你的战略技能的目标时，投D6，若该单位6寸内有一个或多个友军军官模型结果+1，5+得到1CP
- **单位注释**：该单位可以至多被2个领袖单位领导，这2个领袖单位中至多有一个是指挥组单位。如果该单位被2个领袖单位领导，那么当该单位作为护卫被摧毁的时候，那两个领袖单位分别成为原来的两个拥有初始兵力的单位。

## 单位构成
- **10个模型** — 65 分
- **20个模型** — 120 分

## 关键词
- **阵营关键词**：Astra Militarum，Genestealer Cults
- **普通关键词**：[[core-rules/grenades.md|Grenades]]，[[core-rules/battleline.md|Battleline]]，Cadian，Cadian Shock Troops，Platoon，Regiment，[[core-rules/infantry.md|Infantry]]，Imperium