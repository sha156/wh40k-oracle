---
id: '000001041'
name_zh: 死亡守卫终结者巫师
name_en: Death Guard Sorcerer In Terminator Armour
faction: 死亡守卫
type: unit
points:
  1 model: 70
tags:
- unit
- unit/死亡守卫
- 死亡守卫
version:
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| Death Guard Sorcerer In Terminator Armour | 4" | 6 | 2+ | 5 | 6+ | 1 |

### 特殊保护
- 4+

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 复合武器    | 24" | 1 | 4+ | 4 | 0 | 1 | [[core-rules/anti.md\|反步兵 4+]]，[[core-rules/devastating-wounds.md\|毁灭伤害]]，[[core-rules/rapid-fire.md\|速射 1]] |
| 麻风诅咒（标准） | 24" | D6 | 3+ | 6 | -1 | 1 | [[core-rules/psychic-attacks.md\|psychic]] |
| 麻风诅咒（过载） | 24" | 2D6 | 3+ | 6 | -2 | 1 | [[core-rules/hazardous.md\|hazardous]]，[[core-rules/psychic-attacks.md\|psychic]] |
| 瘟疫并联爆弹枪 | 24" | 2 | 3+ | 4 | 0 | 1 | [[core-rules/lethal-hits.md\|致命一击]]，[[core-rules/rapid-fire.md\|速射 2]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 链锯拳 | 近战 | 3 | 4+ | 8 | -2 | 2 | [[core-rules/anti.md\|anti-vehicle 3+]] |
| 灵能武器 | 近战 | 4 | 3+ | 6 | -1 | D3 | [[core-rules/psychic-attacks.md\|psychic]] |
| 瘟疫拳 | 近战 | 3 | 3+ | 8 | -2 | 2 | [[core-rules/lethal-hits.md\|lethal hits]] |
| 瘟疫神尊武器    | 近战 | 4 | 3+ | 5 | -2 | 1 | [[core-rules/lethal-hits.md\|致命一击]] |
| 一对闪电爪 | 近战 | 5 | 3+ | 5 | -2 | 1 | [[core-rules/twin-linked.md\|双联]] |

## 技能
- **【阵营技能】：纳垢赐福【光环】**
- **腐败活力【灵能】**：在近战阶段开始时，你可以投 D6，结果为 1 则本单位遭受 D3 点致命伤害，2+则 持续到本阶段结束，被分配给本单位模型的攻击破 坏力降低 1 点
- **灵能使魔【灵能】**：一次性技能，为本模型的一件 灵能武器选择目标后，本阶段中那件武器的 S 和 D 提高 2 点（灵能使魔的模型只作为标记物存在）

## 单位构成
- **1个模型** — 70 分

## 关键词
- **阵营关键词**：Death Guard
- **普通关键词**：[[core-rules/character.md|Character]]，[[core-rules/infantry.md|Infantry]]，[[core-rules/psyker.md|Psyker]]，Chaos，Nurgle，Terminator，Sorcerer