---
id: '000002588'
name_zh: 克鲁特远猎者小队
name_en: Kroot Farstalkers
faction: 钛帝国
type: unit
points:
  12 models: 75
tags:
- unit
- unit/钛帝国
- 钛帝国
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 远猎者和杀戮应契者 | 7" | 3 | 6+ | 1 | 7+ | 1 |
| 克鲁特猎犬 | 12" | 3 | 6+ | 1 | 7+ | 0 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 多瓦盖皮腺炮 | 12" | D6 | N/A | 4 | -1 | 1 | [[core-rules/ignores-cover.md\|无视掩体]]，[[core-rules/torrent.md\|洪流]] |
| 远猎者火枪 | 24" | 1 | 4+ | 4 | 0 | 1 | [[core-rules/rapid-fire.md\|速射1]] |
| 克鲁特手枪 | 12" | 1 | 4+ | 4 | 0 | 1 | [[core-rules/pistol.md\|手枪]] |
| 隆达希三角弩  | 18" | 3 | 5+ | 7 | -1 | 1 | [[core-rules/anti.md\|针对载具4+]]，[[core-rules/devastating-wounds.md\|毁灭伤害]]，[[core-rules/heavy.md\|重型]] |
| 钛族科技步枪 | 30" | 1 | 4+ | 5 | 0 | 1 | [[core-rules/rapid-fire.md\|速射1]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 格斗武器 | 近战 | 2 | 3+ | 4 | 0 | 1 | — |
| 撕裂牙 | 近战 | 3 | 3+ | 3 | 0 | 1 | — |
| 仪式之刃 | 近战 | 3 | 3+ | 5 | 0 | 1 | — |

## 技能
- **Pech’ra**：Ranged weapons equipped by the bearer’s unit have the [IGNORES COVER] ability.
- **Bounty Hunters**：At the start of the battle, select one unit from your opponent’s army. Each time a model in this unit makes an attack that targets that unit, that attack has the [LETHAL HITS] and [PRECISION] abilities.

## 单位构成
- **12个模型** — 75 分

## 关键词
- **阵营关键词**：T’au Empire
- **普通关键词**：Farstalkers，[[core-rules/grenades.md|Grenades]]，Kroot，[[core-rules/infantry.md|Infantry]]