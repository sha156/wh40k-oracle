---
id: '000000457'
name_en: Manta
faction: 钛帝国
type: unit
tags:
- unit
- unit/钛帝国
- 钛帝国
sources:
- book: Faction Pack Tau Empire
  pages:
  - 17
  - 18
raw:
- data_refined\Faction Pack Tau Empire\page_017.md
- data_refined\Faction Pack Tau Empire\page_018.md
verify_warn: true
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 1 Manta | 40" | 14 | 2+ / 5+ | 60 | 7+ | - |

## 远程武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 重型磁轨炮 | 120" | 1 | 4+ | 26 | -5 | 12 | [[毁灭创伤]] |
| 离子炮 – 标准 | 60" | D6+3 | 4+ | 7 | -1 | 2 | [[爆炸]] |
| 离子炮 – 过载 | 60" | D6+3 | 4+ | 8 | -2 | 3 | [[爆炸]], [[危险]] |
| 长管爆裂炮阵列 | 24" | 32 | 4+ | 6 | -1 | 1 |
| 导弹吊舱 | 30" | 2 | 4+ | 7 | -1 | 2 |
| 寻的导弹 | 48" | 1 | 4+ | 14 | -3 | D6+1 | [[单次射击]] |

- 在为该武器选择目标前，选择它的配置之一来进行攻击。
- [[单次射击]]：持有者每场战斗只能使用此武器射击一次。

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 装甲外壳 | 近战 | 8 | 5+ | 6 | 0 | 1 |

## 技能
- **核心技能**：
  - **[[致命毁灭]] 3D6**：当此模型被摧毁时，掷3个D6，对范围内的单位造成等量致命伤。
  - **[[悬浮]]**：此模型具有悬浮能力。
- **阵营技能**：
  - **[[为了更伟大的上好]]**：适用军队规则。
- **专属技能**：
  - **好斗部署**：在你的[[射击阶段]]，此模型射击后，选择一个被这些攻击中一次或多次命中的敌方单位。直到该阶段结束，每当一个本回合从此运输工具上离舰的友方模型对该敌方单位进行攻击时，你可以重掷[[造伤骰]]。
  - **空军巨像**：每当你以[[计谋]]指定此模型为目标时，你必须支付该计谋所述CP消耗的三倍来这样做。
  - **受损：剩余1-20点Wounds**：当此模型的剩余Wounds为1-20点时，每次此模型进行攻击时，[[命中骰]]的结果减1。

## 单位构成
- **装备选项**：无
- **单位组成**：1 辆 Manta
- **初始装备**：2门重型磁轨炮; 6门离子炮; 2套长管爆裂炮阵列; 2具导弹吊舱; 10枚寻的导弹; 装甲外壳

## 运输能力
此模型的运输容量合计为以下所有：
- 200 个 **帝国卫** **步兵**或**战术无人机** 模型
- 4 辆 **[[factions/钛帝国/units/devilfish.md|魔鬼鱼]]**、**天光导弹艇**或**锤头鲨** 模型
- 8 个Wounds属性为9或更低的**战斗服**模型

## 关键词
- **阵营关键词**：帝国卫
- **普通关键词**：载具、飞行器、飞行、构造体、泰坦、运输、标记光、Manta