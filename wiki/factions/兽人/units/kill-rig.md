---
id: '000002497'
name_zh: 杀戮战车
name_en: Kill Rig
faction: 兽人
type: unit
points:
  1 model: 175
tags:
- unit
- unit/兽人
- 兽人
version:
  points: MFM 2026-09-14T05:51:23.572407+00:00
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-09-14'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 杀戮战车 | 10" | 10 | 3+ | 16 | 7+ | 5 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 重型撸巴炮 | 48" | D6 | 5+ | 6 | 0 | 2 | [[core-rules/blast.md\|爆炸]]，[[core-rules/indirect-fire.md\|曲射]] |
| 鱼叉炮 | 12" | 1 | 5+ | 12 | -2 | 3 | [[core-rules/anti.md\|针对怪物2+]]，[[core-rules/anti.md\|针对载具2+]]，钩缠 |
| [[factions/兽人/units/wurrboy.md\|巫术小子]]塔楼 | 24" | D3 | N/A | 12 | -3 | D6 | [[core-rules/hazardous.md\|危险]]，[[core-rules/psychic-attacks.md\|灵能]]，[[core-rules/torrent.md\|洪流]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 屠夫小子 | 近战 | 4 | 3+ | 5 | -1 | 1 | [[core-rules/anti.md\|针对怪物4+]]，[[core-rules/anti.md\|针对载具4+]]，[[core-rules/extra-attacks.md\|额外攻击]] |
| 大角撞击 | 近战 | 4 | 4+ | 8 | -1 | 3 | [[core-rules/extra-attacks.md\|额外攻击]]，[[core-rules/lance.md\|骑枪]] |
| 电锯刀 | 近战 | 6 | 3+ | 10 | -2 | 2 | — |

## 技能
- **【阵营技能】:WAAAGH!不稳定力量**
- **战争小子（灵能等级 1）**：该模型拥有灵能技能部分列出的灵能技能（下面灵能技能描述）。
- **野兽气息（灵能等级1）**：在您的移动阶段中：当一个搭乘该模型的单位进行脱离移动时，如果该单位没有处于战斗震慑状态，您可以帮一枚 D6 为该单位进行一次灵能掷骰。如果您这样做：•如果掷骰结果为1，该单位处于战斗震慑状态。•在该回合结束前，当那个脱离单位对凶兽/载具单位进行攻击时， 致伤掷骰+1
- **严重损坏**：当本模型W值为1-6时，攻击命中结果-1
- **战争之路（灵能等级 1）**：在近战阶段中，当该单位被选择进行近战时，如果该单位没有处于战斗震慑状态，您可以一枚 D6 为该单位进行一次灵能掷骰。如果您这样做：•如果结果为1，该单位处于战斗震慑状态。•该单位的近战攻击拥有［致命一击］和［灵能。

## 单位构成
- **1个模型** — 175 分

## 关键词
- **阵营关键词**：Orks
- **普通关键词**：[[core-rules/transport.md|Transport]]，Monster，Beast Snagga，Kill Rig，[[core-rules/psyker.md|Psyker]]