---
id: '000000389'
name_zh: 涅墨⻄斯骇骑机甲
name_en: Nemesis Dreadknight
faction: 灰骑士
type: unit
points:
  1 model: 195
tags:
- unit
- unit/灰骑士
- 灰骑士
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 涅墨⻄斯骇骑机甲 | 8" | 8 | 2+ | 13 | 6+ | 4 |

### 特殊保护
- 4+

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 加特林消灵炮 | 24" | 12 | 3+ | 6 | 0 | 1 | [[core-rules/psychic-attacks.md\|灵能]]，[[core-rules/sustained-hits.md\|连击1]] |
| 焚化炮 | 18" | 2D6 | N/A | 6 | -1 | 1 | ⽆视掩体，[[core-rules/torrent.md\|洪流]] |
| 重型灵能炮 | 24" | 6 | 3+ | 10 | -2 | 3 | [[core-rules/psychic-attacks.md\|灵能]]，⽆视掩体 |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 恐惧铁拳 | 近战 | 6 | 2+ | 6 | -1 | 1 | — |
| 天罚屠魔巨锤 | 近战 | 5 | 3+ | 14 | -3 | D6+1 | [[core-rules/psychic-attacks.md\|灵能]] |
| 天罚巨剑（重击） | 近战 | 5 | 2+ | 10 | -2 | D6 | [[core-rules/psychic-attacks.md\|灵能]] |
| 天罚巨剑（横扫） | 近战 | 10 | 2+ | 5 | -1 | 1 | [[core-rules/psychic-attacks.md\|灵能]] |

## 技能
- **【阵营技能】无限之门**
- **不屈精神（灵能）**：该模型可在进⾏过突进或后撤的回合中进⾏射击和宣布冲锋。
- **损伤**：当该模型剩余 1-4 点耐伤时，它的攻击命中掷骰结果减少 1 点。

## 单位构成
- **1个模型** — 195 分

## 关键词
- **阵营关键词**：Grey Knights
- **普通关键词**：Nemesis Dreadknight，[[core-rules/psyker.md|Psyker]]，[[core-rules/walker.md|Walker]]，Vehicle，Imperium