...
## 技能
- **【阵营技能】：痛苦之力**：无处可违[苦痛]:在您的射击阶段，当您选择该模型所在的单位进行射击时，您可以花费1枚苦痛标识来强化该单位。在该单位处于强化状态时，在该单位完成射击后，选择一个被其一次或多次攻击命中的敌方单位(凶兽和载具单位除外);直到您的[[factions/兽人/stratagems/on-to-da-next.md|下一个]]回合开始，该敌方单位被压制。当一个单位被压制时，其移动属性减少2”,并且为该单位进行的冲锋抑假结果减少2点。
- **空对地战机**：当该模型对敌方单位（可飞行单位除外） 进行射击攻击时，命中掷骰的结果增加 1 点。
...
## 关键词
- **阵营关键词**：Drukhari
- **普通关键词**：Razorwing Jetfighter，Aeldari，[[core-rules/aircraft.md|Aircraft]]，Fly，Vehicle