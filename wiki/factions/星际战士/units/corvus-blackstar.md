---
id: '000000358'
name_zh: 黑星渡鸦
name_en: Corvus Blackstar
faction: 星际战士
type: unit
points:
  1 model: 180
tags:
- unit
- unit/星际战士
- 星际战士
version:
  points: MFM 2026-09-14T05:51:23.572407+00:00
  source: official-db
sources:
- book: 'Faction Pack: Deathwatch'
  pages:
  - 16
- book: 'Faction Pack: Deathwatch'
  pages:
  - 17
updated: '2026-09-14'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 黑星渡鸦 | 14" | 10 | 3+ | 14 | 6+ | 0 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 黑星火箭发射器 | 30" | D6+1 | 3+ | 5 | 0 | 1 | [[core-rules/blast.md\|爆炸]] |
| 飓风爆矢枪 | 24" | 6 | 3+ | 4 | 0 | 1 | [[core-rules/rapid-fire.md\|速射6]]，[[core-rules/twin-linked.md\|双联]] |
| 风暴突击导弹发射器 | 48" | 1 | 3+ | 10 | -2 | 3 | — |
| 双联突击炮 | 24" | 6 | 3+ | 6 | 0 | 1 | [[core-rules/devastating-wounds.md\|毁灭伤害]]，[[core-rules/twin-linked.md\|双联]] |
| 双联激光炮 | 48" | 1 | 3+ | 12 | -3 | D6+1 | [[core-rules/twin-linked.md\|双联]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 装甲外壳 | 近战 | 3 | 4+ | 6 | 0 | 1 | — |

## 技能
- **黑星集群发射器**：当该模型进行常规移动后，您可以选择一个该模型移动越过的敌方单位，并掷六枚D6；每有一个结果为 5+ 的掷骰，该敌方单位受到1 处致命伤。
- **占卜阵列**：持有者装备的远程武器拥有 [无视掩体] 技能。
- **地狱光环发射器**：持有者拥有烟幕关键词。
- **【阵营技能】：破敌重誓，任务战术**

## 单位构成
- **1个模型** — 180 分

## 关键词
- **阵营关键词**：Deathwatch，Adeptus Astartes
- **普通关键词**：Corvus Blackstar，Vehicle，Imperium，[[core-rules/transport.md|Transport]]，[[core-rules/aircraft.md|Aircraft]]，Fly，FRAME