---
id: '000000091'
name_zh: 射手型无畏机甲
name_en: Ballistus Dreadnought
faction: 星际战士
type: unit
points:
  1 model: 150
tags:
- unit
- unit/星际战士
- 星际战士
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| Ballistus [[factions/星际战士/units/dreadnought.md\|Dreadnought]] | 8" | 10 | 2+ | 12 | 6+ | 4 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 射手导弹发射器（破片） | 48" | 2D6 | 3+ | 5 | 0 | 1 | [[core-rules/blast.md\|blast]] |
| 射手导弹发射器（穿甲） | 48" | 2 | 3+ | 10 | -2 | D6 | — |
| 射手激光炮 | 48" | 2 | 3+ | 12 | -3 | D6+1 | — |
| 双联风暴爆弹枪  | 24" | 2 | 3+ | 4 | 0 | 1 | [[core-rules/rapid-fire.md\|速射 2]]，[[core-rules/twin-linked.md\|双联]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 装甲铁脚 | 近战 | 5 | 3+ | 7 | 0 | 1 | — |

## 技能
- **严重损伤**：本模型 W 值为 1-4 时，攻击命中结果-1
- **【阵营技能】：破敌重誓**
- **    射手轰炸**：本模型对没有低于半数的目标射击时可以重投命中结果

## 单位构成
- **1个模型** — 150 分

## 关键词
- **阵营关键词**：Adeptus Astartes
- **普通关键词**：Ballistus [[factions/星际战士/units/dreadnought.md|Dreadnought]]，Deathwing，[[core-rules/walker.md|Walker]]，Vehicle，Dreadnought，Imperium