---
id: 000000094
name_zh: 摩托牧师
name_en: Chaplain On Bike
faction: 星际战士
type: unit
points:
  1 model: 70
tags:
- unit
- unit/星际战士
- 星际战士
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| [[factions/星际战士/units/chaplain.md\|Chaplain]] On Bike | 12" | 5 | 3+ | 5 | 5+ | 1 |

### 特殊保护
- 4+

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 赦免者爆弹手枪   | 18" | 1 | 3+ | 5 | -1 | 2 | [[core-rules/pistol.md\|手枪]] |
| 双联爆弹步枪    | 24" | 2 | 3+ | 4 | -1 | 1 | [[core-rules/twin-linked.md\|双联]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 牧师权杖     | 近战 | 5 | 2+ | 6 | -1 | 2 | — |

## 技能
- **【阵营技能】：破敌重誓**
- **憎恨祷言**：本模型所领导单位中的模型近战攻击造伤结果+1
- **烈火教典**：每当本模型所在单位被选中进行射击时，你可以选择本模型视线中 12 寸内一个敌方单位，本单位的射击武器在本阶段对那个敌方单位攻击时获得【毁灭伤害】技能

## 单位构成
- **1个模型** — 70 分

## 关键词
- **阵营关键词**：Adeptus Astartes
- **普通关键词**：Ravenwing，[[core-rules/mounted.md|Mounted]]，Chaplain，Imperium，[[core-rules/grenades.md|Grenades]]，[[core-rules/character.md|Character]]