---
id: '000001191'
name_zh: 暴风渡鸦炮艇
name_en: Stormraven Gunship
faction: 星际战士
type: unit
points:
  1 model: 280
tags:
- unit
- unit/星际战士
- 星际战士
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| Stormraven Gunship | 20+" | 10 | 3+ | 14 | 6+ | 0 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 飓风爆弹阵列 | 24" | 6 | 3+ | 4 | 0 | 1 | [[core-rules/rapid-fire.md\|rapid fire 6]]，[[core-rules/twin-linked.md\|twin-linked]] |
| 风暴突击导弹发射器 | 48" | 1 | 3+ | 10 | -2 | 3 | — |
| 双联突击炮 | 24" | 6 | 3+ | 6 | 0 | 1 | [[core-rules/devastating-wounds.md\|devastating wounds]]，[[core-rules/twin-linked.md\|twin-linked]] |
| 双联重型爆弹枪 | 36" | 3 | 3+ | 5 | -1 | 2 | [[core-rules/sustained-hits.md\|sustained hits 1]]，[[core-rules/twin-linked.md\|twin-linked]] |
| 双联重型等离子炮（标准） | 36" | D3 | 3+ | 7 | -2 | 2 | [[core-rules/blast.md\|blast]]，[[core-rules/twin-linked.md\|twin-linked]] |
| 双联重型等离子炮（过载） | 36" | D3 | 3+ | 8 | -3 | 3 | [[core-rules/blast.md\|blast]]，[[core-rules/hazardous.md\|hazardous]]，[[core-rules/twin-linked.md\|twin-linked]] |
| 双联激光炮 | 48" | 1 | 3+ | 12 | -3 | D6+1 | [[core-rules/twin-linked.md\|twin-linked]] |
| 双联多管热熔 | 18" | 2 | 3+ | 9 | -4 | D6 | [[core-rules/melta.md\|melta 2]]，[[core-rules/twin-linked.md\|twin-linked]] |
| 台风导弹发射器（破片） | 48" | 2D6 | 3+ | 4 | 0 | 1 | [[core-rules/blast.md\|blast]] |
| 台风导弹发射器（穿甲） | 48" | 2 | 3+ | 9 | -2 | D6 | — |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 装甲外壳 | 近战 | 6 | 4+ | 8 | 0 | 1 | — |

## 技能
- **Armoured Resilience**：Each time an attack is allocated to this model, subtract 1 from the Damage characteristic of that attack.

## 单位构成
- **1个模型** — 280 分

## 关键词
- **阵营关键词**：Adeptus Astartes
- **普通关键词**：Fly，Vehicle，[[core-rules/aircraft.md|Aircraft]]，Ravenwing，Stormraven Gunship，Imperium，[[core-rules/transport.md|Transport]]