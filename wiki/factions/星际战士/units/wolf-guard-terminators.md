---
id: '000000318'
name_zh: 野狼守卫终结者
name_en: Wolf Guard Terminators
faction: 星际战士
type: unit
points:
  5 models: 150
  10 models: 300
tags:
- unit
- unit/星际战士
- 星际战士
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| [[factions/星际战士/units/wolf-guard.md\|Wolf Guard]] Terminators | 6" | 5 | 2+ | 3 | 6+ | 1 |

### 特殊保护
- 4+

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 突击炮 | 24" | 6 | 2+ | 6 | 0 | 1 | [[core-rules/devastating-wounds.md\|毁灭伤害]] |
| 风暴爆弹枪  | 24" | 2 | 2+ | 4 | 0 | 1 | [[core-rules/rapid-fire.md\|速射 2]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 精工动力武器 | 近战 | 4 | 3+ | 5 | -2 | 2 | — |
| 动力拳   | 近战 | 3 | 3+ | 8 | -2 | 2 | — |
| 遗物巨斧  | 近战 | 3 | 3+ | 7 | -2 | 3 | [[core-rules/devastating-wounds.md\|毁灭伤害]] |
| 一对闪电爪    | 近战 | 6 | 3+ | 5 | -2 | 1 | [[core-rules/twin-linked.md\|双联]] |

## 技能
- **【阵营技能】：破敌重誓**
- **粗犷坚韧**：每当攻击被分配给本单位时，如果那些 攻击的 S 值高于本单位的 T 值，则那些攻击的造 伤骰结果-1。
- **风暴盾【装备】**：装备者 W 值变为 4。

## 单位构成
- **5个模型** — 150 分
- **10个模型** — 300 分

## 关键词
- **阵营关键词**：Space Wolves，Adeptus Astartes
- **普通关键词**：[[core-rules/infantry.md|Infantry]]，Imperium，Terminator，[[factions/星际战士/units/wolf-guard.md|Wolf Guard]]，Wolf Guard Terminators