---
id: '000002714'
name_zh: 歼灭者型猎食者坦克
name_en: Predator Annihilator
faction: 星际战士
type: unit
points:
  1 model: 135
tags:
- unit
- unit/星际战士
- 星际战士
version:
  points: MFM 2026-09-14T05:51:23.572407+00:00
  source: official-db
sources:
- book: https://assets.warhammer-community.com/eng_wh40k_faction_pack_space_marines-kxoxqpsahz-u0lzirv0zl.pdf
  pages:
  - 61
updated: '2026-09-14'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| Predator Annihilator | 10" | 10 | 3+ | 11 | 6+ | 3 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 猎食者双联激光炮 | 48" | 1 | 3+ | 14 | -3 | D6+1 | [[core-rules/twin-linked.md\|双联]] |
| 重型爆弹枪 | 36" | 3 | 3+ | 5 | -1 | 2 | [[core-rules/sustained-hits.md\|连击1]] |
| 猎杀飞弹    | 48" | 1 | 2+ | 14 | -3 | D6 | [[core-rules/one-shot.md\|单发]] |
| 激光炮  | 48" | 1 | 3+ | 12 | -3 | D6+1 | — |
| 风暴爆弹枪 | 24" | 2 | 3+ | 4 | 0 | 1 | [[core-rules/rapid-fire.md\|速射2]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 装甲履带   | 近战 | 3 | 4+ | 6 | 0 | 1 | — |

## 技能
- **【阵营技能】：破敌重誓**
- **歼灭者**：本模型的射击攻击被分配给巨兽或载具单位时，你可以重投破坏力（D）
- **严重损伤**：本模型 W 值为 1-4 时，攻击命中结果-1

## 单位构成
- **1个模型** — 135 分

## 关键词
- **阵营关键词**：Adeptus Astartes
- **普通关键词**：Vehicle，[[core-rules/smoke.md|Smoke]]，Imperium，Predator Annihilator，FRAME