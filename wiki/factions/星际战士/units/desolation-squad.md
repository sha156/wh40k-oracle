---
id: '000002270'
name_zh: 寂灭者小队
name_en: Desolation Squad
faction: 星际战士
type: unit
points:
  5 models: 180
tags:
- unit
- unit/星际战士
- 星际战士
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| Desolation Squad | 6" | 4 | 3+ | 2 | 6+ | 1 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 爆弹手枪 | 12" | 1 | 3+ | 4 | 0 | 1 | [[core-rules/pistol.md\|手枪]] |
| 堡主火箭发射器 | 36" | D3 | 3+ | 4 | 0 | 1 | [[core-rules/blast.md\|blast]]，[[core-rules/indirect-fire.md\|indirect fire]] |
| 高能破片火箭 | 48" | D6+1 | 4+ | 5 | 0 | 1 | [[core-rules/blast.md\|blast]]，[[core-rules/heavy.md\|heavy]] |
| 高能穿甲火箭 | 48" | 1 | 4+ | 10 | -2 | D6+1 | [[core-rules/heavy.md\|heavy]] |
| 威格尔火箭  | 48" | D6 | 2+ | 7 | -1 | 2 | [[core-rules/blast.md\|爆炸]]，[[core-rules/indirect-fire.md\|曲射]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 格斗武器 | 近战 | 3 | 3+ | 4 | 0 | 1 | — |

## 技能
- **光学瞄准器**：如果本单位保持静止，则直到下个你的移动阶段开始时，本单位模型的射击武器获得【忽视掩体】技能
- **【阵营技能】：破敌重誓**

## 单位构成
- **5个模型** — 180 分

## 关键词
- **阵营关键词**：Adeptus Astartes
- **普通关键词**：[[core-rules/grenades.md|Grenades]]，[[core-rules/infantry.md|Infantry]]，Desolation Squad，Tacticus，Imperium