---
id: '000002712'
name_zh: 先遣者摩托小队
name_en: Outrider Squad
faction: 星际战士
type: unit
points:
  3 models: 70
  6 models: 140
  Invader ATV: 60
tags:
- unit
- unit/星际战士
- 星际战士
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| OUTRIDER | 12" | 5 | 3+ | 4 | 6+ | 2 |
| INVADER ATV | 12" | 5 | 3+ | 8 | 6+ | 2 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 爆弹手枪 | 12" | 1 | 3+ | 4 | 0 | 1 | [[core-rules/pistol.md\|手枪]] |
| 重型爆弹手枪   | 18" | 1 | 3+ | 4 | -1 | 1 | [[core-rules/pistol.md\|手枪]] |
| 多管热熔 | 18" | 2 | 3+ | 9 | -4 | D6 | [[core-rules/melta.md\|melta 2]] |
| 突击加特林 | 24" | 8 | 3+ | 5 | 0 | 1 | [[core-rules/devastating-wounds.md\|毁灭伤害]] |
| 双联爆弹步枪 | 24" | 2 | 3+ | 4 | -1 | 1 | [[core-rules/twin-linked.md\|twin-linked]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 阿斯塔特链锯剑 | 近战 | 4 | 3+ | 4 | -1 | 1 | — |
| 格斗武器  | 近战 | 5 | 3+ | 4 | 0 | 1 | — |

## 技能
- **【阵营技能】：破敌重誓**
- **雷霆冲撞**：每当本单位模型进行近战攻击时，如果本回合进行过冲锋移动，则本轮攻击 S 和 D 都+1

## 单位构成
- **3个模型** — 70 分
- **6个模型** — 140 分
- **[[factions/星际战士/units/invader-atv.md|Invader ATV]]** — 60 分

## 关键词
- **阵营关键词**：Adeptus Astartes
- **普通关键词**：Imperium，Outrider Squad，Ravenwing，[[core-rules/mounted.md|Mounted]]，[[core-rules/grenades.md|Grenades]]