---
id: '000004131'
name_zh: 狼卫猎头者
name_en: Wolf Guard Headtakers
faction: 星际战士
type: unit
points:
  3 Wolf Guard Headtakers: 85
  3 Wolf Guard Headtakers and 3 Hunting Wolves: 110
  6 Wolf Guard Headtakers: 170
  6 Wolf Guard Headtakers and 6 Hunting Wolves: 220
  3 wolf guard headtakers, 3 hunting wolves: 115
  6 wolf guard headtakers, 6 hunting wolves: 230
tags:
- unit
- unit/星际战士
- 星际战士
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 狼卫 | 7" | 4 | 3+ | 3 | 6+ | 1 |
| 猎狼 | 10" | 4 | 6+ | 1 | 8+ | 0 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 重型爆弹手枪 | 18" | 1 | 3+ | 4 | -1 | 1 | [[core-rules/pistol.md\|pistol]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 精工动力武器    | 近战 | 4 | 3+ | 5 | -2 | 2 | — |
| 一对精工动力武器   | 近战 | 6 | 3+ | 5 | -2 | 2 | — |
| 尖牙与利爪    | 近战 | 3 | 4+ | 4 | 0 | 1 | — |

## 技能
- **【阵营技能】：破敌重誓**
- **猎狼脱缰**：在你的军表构成阶段，如果本单位包含猎狼模 型，则将本单位分为两个新单位，其中一个包含所有的狼卫猎头者模型，另一个包含所有的猎狼模型，这些单位根 据其对应模型的数量获得全新的初始力量值。
- **猎首者**：在战斗开始时，选择您对手军队中的一个单位作为该单位的猎物。当对猎物进行攻击时，该单位中夺首者模型装备的武器拥有[毁灭伤害]和[精准]技能。每当该单位的猎物被摧毁时，选择一个新的敌方单位作为该单位的猎物。即使该单位搭乘在运输工具中，其依然可以使用该技能。
- **狩猎伴狼**：当本单位位于一个或多个太空野狼人物模型 6 寸内（狼人模型除外）时，如果本单位尚未被战斗震慑， 则本单位中的猎狼模型 OC 值变为 1。
- **风暴盾【装备】**：装备者获得 4+特殊保护

## 单位构成
- **3 [[factions/星际战士/units/wolf-guard.md|Wolf Guard]] Headtakers** — 85 分
- **3 [[factions/星际战士/units/wolf-guard.md|Wolf Guard]] Headtakers and 3 Hunting Wolves** — 110 分
- **6 Wolf Guard Headtakers** — 170 分
- **6 Wolf Guard Headtakers and 6 Hunting Wolves** — 220 分
- **3 wolf guard headtakers, 3 hunting wolves** — 115 分
- **6 wolf guard headtakers, 6 hunting wolves** — 230 分

## 关键词
- **阵营关键词**：Adeptus Astartes，Space Wolves
- **普通关键词**：Imperium，Beasts，Headtakers，Wolf Guard，Tacticus，Imperium，[[core-rules/infantry.md|Infantry]]，Hunting Wolves