---
id: '000000060'
name_zh: 生物学药剂师
name_en: Apothecary Biologis
faction: 星际战士
type: unit
points:
  1 model: 70
tags:
- unit
- unit/星际战士
- 星际战士
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| [[factions/星际战士/units/apothecary.md\|Apothecary]] Biologis | 5" | 6 | 3+ | 5 | 6+ | 3 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 赦免者爆弹手枪     | 18" | 1 | 3+ | 5 | -1 | 2 | [[core-rules/pistol.md\|手枪]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 格斗武器   | 近战 | 4 | 3+ | 4 | 0 | 1 | — |

## 技能
- **【阵营技能】：破敌重誓**
- **手术式精准**：本模型所领导的单位所装备的武器获得【致命一击】技能
- **采集生物样本**：当本模型所在单位近战消灭一个敌方单位时，本模型的 OC 值在本场游戏中永久提高到 9

## 单位构成
- **1个模型** — 70 分

## 关键词
- **阵营关键词**：Adeptus Astartes
- **普通关键词**：[[factions/星际战士/units/apothecary.md|Apothecary]]，[[core-rules/infantry.md|Infantry]]，[[core-rules/character.md|Character]]，Imperium，Gravis，Biologis