---
id: '000000120'
name_en: Venerable Dreadnought (Legendary)
faction: 星际战士
type: unit
points:
  1 model: 165
tags:
- unit
- unit/星际战士
- 星际战士
version:
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| [[factions/星际战士/units/venerable-dreadnought.md\|Venerable Dreadnought]] | 6" | 9 | 2+ | 8 | 6+ | 3 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| Assault cannon | 24" | 6 | 3+ | 6 | 0 | 1 | devastating wounds |
| [[factions/星际战士/units/dreadnought.md\|Dreadnought]] inferno cannon | 12" | 2D6 | N/A | 6 | -1 | 1 | ignores cover，torrent |
| Heavy flamer | 12" | D6 | N/A | 5 | -1 | 1 | ignores cover，torrent |
| Heavy plasma cannon – standard | 36" | D3 | 3+ | 7 | -2 | 2 | [[core-rules/blast.md\|BLAST]] |
| Heavy plasma cannon – supercharge | 36" | D3 | 3+ | 8 | -3 | 3 | [[core-rules/blast.md\|BLAST]]，[[core-rules/hazardous.md\|HAZARDOUS]] |
| Missile launcher – frag | 48" | D6 | 3+ | 4 | 0 | 1 | [[core-rules/blast.md\|BLAST]] |
| Missile launcher – krak | 48" | 1 | 3+ | 9 | -2 | D6 | — |
| Multi-melta | 18" | 2 | 3+ | 9 | -4 | D6 | [[core-rules/melta.md\|melta 2]] |
| Storm bolter | 24" | 2 | 3+ | 4 | 0 | 1 | [[core-rules/rapid-fire.md\|rapid fire 2]] |
| Twin autocannon | 48" | 2 | 3+ | 9 | -1 | 3 | twin-linked |
| Twin heavy bolter | 36" | 3 | 3+ | 5 | -1 | 2 | [[core-rules/sustained-hits.md\|sustained hits 1]]，twin-linked |
| Twin heavy flamer | 12" | D6 | N/A | 5 | -1 | 1 | ignores cover，torrent，twin-linked |
| Twin lascannon | 48" | 1 | 3+ | 12 | -3 | D6+1 | twin-linked |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| Armoured feet | 近战 | 5 | 3+ | 6 | 0 | 1 | — |
| Dreadnought combat weapon | 近战 | 5 | 3+ | 12 | -2 | 3 | — |

## 技能
- **Wisdom of the [[factions/星际战士/units/ancient.md|Ancient]]s (Aura)**：While a friendly Adeptus Astartes Infantry unit is within 6" of this model, each time a model in that unit makes an attack, re-roll a Hit roll of 1.

## 单位构成
- **1个模型** — 165 分

## 关键词
- **阵营关键词**：Adeptus Astartes
- **普通关键词**：Venerable，Dreadnought，Deathwing，[[core-rules/walker.md|Walker]]，Vehicle，[[core-rules/smoke.md|Smoke]]，Imperium