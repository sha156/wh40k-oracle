---
id: '000001157'
name_zh: 仲裁者小队
name_en: Intercessor Squad
faction: 星际战士
type: unit
points:
  5 models: 80
  10 models: 150
tags:
- unit
- unit/星际战士
- 星际战士
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| Intercessor Squad | 6" | 4 | 3+ | 2 | 6+ | 2 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 阿斯塔特榴弹发射器（破片）     | 24" | D3 | 3+ | 4 | 0 | 1 | [[core-rules/blast.md\|爆炸1]] |
| 阿斯塔特榴弹发射器（穿甲） | 24" | 1 | 3+ | 9 | -2 | D3 | — |
| 爆弹手枪    | 12" | 1 | 3+ | 4 | 0 | 1 | [[core-rules/pistol.md\|手枪]] |
| 爆弹步枪    | 24" | 2 | 3+ | 4 | -1 | 1 | [[core-rules/assault.md\|突击]]，[[core-rules/heavy.md\|重型]] |
| 喷火手枪     | 12" | D6 | N/A | 3 | 0 | 1 | [[core-rules/ignores-cover.md\|忽视掩体]]，[[core-rules/pistol.md\|手枪]]，[[core-rules/torrent.md\|喷射]] |
| 等离子手枪（标准）  | 12" | 1 | 3+ | 7 | -2 | 1 | [[core-rules/pistol.md\|手枪]] |
| 等离子手枪（过载） | 12" | 1 | 3+ | 8 | -3 | 2 | [[core-rules/hazardous.md\|hazardous]]，[[core-rules/pistol.md\|pistol]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 阿斯塔特链锯剑 | 近战 | 5 | 3+ | 4 | -1 | 1 | — |
| 格斗武器   | 近战 | 3 | 3+ | 4 | 0 | 1 | — |
| 动力拳 | 近战 | 3 | 3+ | 8 | -2 | 2 | — |
| 动力武器  | 近战 | 4 | 3+ | 5 | -2 | 1 | — |
| 雷霆锤 | 近战 | 3 | 4+ | 8 | -2 | 2 | [[core-rules/devastating-wounds.md\|devastating wounds]] |

## 技能
- **【阵营技能】：破敌重誓**
- **目标确保**：如果在你的指挥阶段结束时，本单位位于某个你占领的目标点范围内，则即使你在这个目标点范围内已没有你的模型，也算作你占领此目标点，此效果持续到敌方占领此目标点后，任意回合开始或结束时为止
- **目标歼灭**：每当本单位被选中进行射击时，只选择了一个目标，就可以使用本技能，让本阶段中所装备的爆弹步枪A+2

## 单位构成
- **5个模型** — 80 分
- **10个模型** — 150 分

## 关键词
- **阵营关键词**：Adeptus Astartes
- **普通关键词**：[[core-rules/battleline.md|Battleline]]，[[core-rules/grenades.md|Grenades]]，Intercessor Squad，Tacticus，Imperium，[[core-rules/infantry.md|Infantry]]