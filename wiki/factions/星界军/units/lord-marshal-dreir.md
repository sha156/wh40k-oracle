---
id: 000003890
name_zh: 大元帅德雷尔
name_en: Lord Marshal Dreir
faction: 星界军
type: unit
points:
  1 model: 75
tags:
- unit
- unit/星界军
- 星界军
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 大元帅德雷尔 | 10" | 4 | 4+ | 6 | 7+ | 2 |

### 特殊保护
- 4+

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 激光手枪 | 12" | 1 | 3+ | 3 | 0 | 1 | [[core-rules/pistol.md\|手枪]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 牺牲军刀 | 近战 | 6 | 2+ | 6 | -2 | 1 | [[core-rules/anti.md\|针对步兵4+]] |
| 马蹄践踏 | 近战 | 2 | 4+ | 4 | -1 | 1 | [[core-rules/extra-attacks.md\|额外攻击]] |

## 技能
- **阵营能力-指挥之声**：该军官可以对至多3个兵团单位下达命令
- **耐杀王**：当该模型首次被摧毁时，在本阶段结束时投掷一颗D6，结果为2+则尽可能的将该模型放置在被摧毁的地点并且不处于任何敌军交战范围内并回复D3点耐伤
- **引领冲锋**：当该模型所在单位进行一次冲锋移动时，直到本回合结束时该模型所在单位的模型的近战武器获得毁灭性伤害能力
- **特殊保护**：该模型拥有4+特殊保护

## 单位构成
- **1个模型** — 75 分

## 关键词
- **阵营关键词**：Astra Militarum
- **普通关键词**：[[core-rules/grenades.md|Grenades]]，Lord Marshal Dreir，Officer，Imperium，[[core-rules/mounted.md|Mounted]]，[[core-rules/character.md|Character]]，[[core-rules/epic-hero.md|Epic Hero]]