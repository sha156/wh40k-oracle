---
id: '000000700'
name_zh: 黎曼鲁斯战斗坦克
name_en: Leman Russ Battle Tank
faction: 星界军
type: unit
points:
  1 model: 160
tags:
- unit
- unit/星界军
- 星界军
version:
  points: MFM 2026-09-14T05:51:23.572407+00:00
  source: official-db
sources:
- book: https://assets.warhammer-community.com/eng_wh40k_faction_pack_astra_militarum-o51zhqvsvy-wdobxbf6t0.pdf
  pages:
  - 23
updated: '2026-09-14'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 黎曼鲁斯战斗坦克 | 10" | 11 | 2+ | 13 | 7+ | 3 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 重型爆弹枪 | 36" | 3 | 4+ | 5 | -1 | 2 | [[core-rules/sustained-hits.md\|连击1]] |
| 重型火焰喷射器 | 12" | D6 | N/A | 5 | -1 | 1 | [[core-rules/ignores-cover.md\|无视掩体]]，[[core-rules/torrent.md\|洪流]] |
| 重型伐木枪 | 36" | 3 | 4+ | 4 | 0 | 1 | [[core-rules/rapid-fire.md\|速射3]] |
| 猎杀飞弹 | 48" | 1 | 4+ | 14 | -3 | D6 | [[core-rules/one-shot.md\|单发]] |
| 激光炮 | 48" | 1 | 4+ | 12 | -3 | D6+1 | — |
| 黎曼鲁斯战斗加农炮 | 48" | D6+3 | 4+ | 10 | -1 | 3 | [[core-rules/blast.md\|爆炸]] |
| 多管热熔 | 18" | 2 | 4+ | 9 | -4 | D6 | [[core-rules/melta.md\|热熔2]] |
| 等离子炮（标准） | 36" | D3 | 4+ | 7 | -2 | 1 | [[core-rules/blast.md\|爆炸]] |
| 等离子炮（过载） | 36" | D3 | 4+ | 8 | -3 | 2 | [[core-rules/blast.md\|爆炸]]，[[core-rules/hazardous.md\|危险]] |
| 风暴爆弹枪 | 24" | 2 | 4+ | 4 | 0 | 1 | [[core-rules/rapid-fire.md\|速射2]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 装甲履带 | 近战 | 6 | 4+ | 7 | 0 | 1 | — |

## 技能
- **装甲矛头**：当该单位攻击敌军单位时，重投结果为1的命中骰。如果该敌军单位位于一个你未控制的目标标记范围内，你可以重投所有命中骰。
- **受损**：当该模型剩余1-4点耐伤，该模型进行攻击时命中投掷结果-1。

## 单位构成
- **1个模型** — 160 分

## 关键词
- **阵营关键词**：Astra Militarum
- **普通关键词**：Imperium，[[core-rules/smoke.md|Smoke]]，Vehicle，Squadron，Leman Russ Battle Tank，FRAME