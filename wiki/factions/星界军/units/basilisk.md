---
id: '000000695'
name_zh: 石化蜥蜴自行火炮
name_en: Basilisk
faction: 星界军
type: unit
points:
  1 model: 115
tags:
- unit
- unit/星界军
- 星界军
version:
  points: MFM 2026-09-14T05:51:23.572407+00:00
  source: official-db
sources:
- book: https://assets.warhammer-community.com/eng_wh40k_faction_pack_astra_militarum-o51zhqvsvy-wdobxbf6t0.pdf
  pages:
  - 23
updated: '2026-09-14'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 石化蜥蜴自行火炮 | 10" | 9 | 3+ | 11 | 7+ | 3 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 撼地炮 | 240" | D6+3 | 4+ | 8 | -2 | 2 | [[core-rules/blast.md\|爆炸]]，[[core-rules/indirect-fire.md\|曲射]] |
| 重型爆弹枪 | 36" | 3 | 4+ | 5 | -1 | 2 | [[core-rules/sustained-hits.md\|连击1]] |
| 重型火焰喷射器 | 12" | D6 | N/A | 5 | -1 | 1 | [[core-rules/ignores-cover.md\|无视掩体]]，[[core-rules/torrent.md\|洪流]] |
| 猎杀飞弹 | 48" | 1 | 4+ | 14 | -3 | D6 | [[core-rules/one-shot.md\|单发]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 装甲履带 | 近战 | 3 | 4+ | 6 | 0 | 1 | — |

## 技能
- **撼地炮击**：在您的射击阶段中，在该模型进行射击后，如果一个敌方步兵单位被该模型撼地炮的一次或多次攻击命中，在您的[[factions/兽人/stratagems/on-to-da-next.md|下一个]]射击阶段开始前，那个单位被震撼。当单位处于震撼状态时，移动属性降低2”，并且它进行的冲锋掷骰结果减2”。
- **受损**：当该模型剩余1-4点耐伤，该模型进行攻击时命中投掷结果-1。

## 单位构成
- **1个模型** — 115 分

## 关键词
- **阵营关键词**：Astra Militarum
- **普通关键词**：Vehicle，Artillery，[[core-rules/smoke.md|Smoke]]，Squadron，Basilisk，Imperium，FRAME