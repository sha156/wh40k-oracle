---
id: 000003852
name_zh: 恶角野兽人
name_en: Fellgor Beastmen
faction: 混沌骑士
type: unit
points:
  10 models: 70
tags:
- unit
- unit/混沌骑士
- 混沌骑士
version:
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| Fellgor Beastmen | 6" | 4 | 5+ | 1 | 7+ | 1 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 等离子手枪（过载） | 12" | 1 | 4+ | 8 | -3 | 2 | [[core-rules/pistol.md\|手枪]]，[[core-rules/hazardous.md\|危险]] |
| 自动手枪 | 12" | 1 | 4+ | 3 | 0 | 1 | [[core-rules/pistol.md\|手枪]] |
| 爆弹手枪 | 12" | 1 | 4+ | 4 | 0 | 1 | [[core-rules/pistol.md\|手枪]] |
| Corrupted stave | 18" | D3 | 4+ | 5 | -1 | 1 | devastating wounds，psychic |
| 等离子手枪（标准） | 12" | 1 | 4+ | 7 | -2 | 1 | [[core-rules/pistol.md\|手枪]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 重型近战武器     | 近战 | 2 | 5+ | 8 | -1 | 2 | — |
| 格斗武器 | 近战 | 2 | 4+ | 4 | 0 | 1 | — |
| 链锯剑 | 近战 | 3 | 4+ | 4 | 0 | 1 | — |
| 腐化法杖  | 近战 | 2 | 4+ | 4 | -1 | D3 | [[core-rules/devastating-wounds.md\|毁灭伤害]]，灵能 |

## 技能
- **【阵营技能】：黑暗契约**：：每当一个敌方单位射击造成本单位至少 一个模型被消灭时，本单位如果不处于震慑状态可 以进行一次最多 D6 寸的特殊移动，但是这次移动必 须尽可能的更靠近最近的敌方单位（飞行器单位除 外），甚至可以进入敌方单位的接战范围
- **兽化[[factions/黑暗灵族/units/reavers.md|劫掠者]]**：如果本单位在游戏开始时处于战略预备队 中，则可以在你的第 1-3 回合的援军入场子阶段部署入 场，并且入场时算作比当前游戏大回合数多一回合

## 单位构成
- **10个模型** — 70 分

## 关键词
- **阵营关键词**：Heretic Astartes，Chaos Knights
- **普通关键词**：[[core-rules/infantry.md|Infantry]]，[[core-rules/grenades.md|Grenades]]，Chaos，Damned，Fellgor Beastmen