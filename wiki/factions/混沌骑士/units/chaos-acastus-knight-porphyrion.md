---
id: 000001099
name_zh: 混沌巨人王型阿卡斯托斯骑士
name_en: Chaos Acastus Knight Porphyrion
faction: 混沌骑士
type: unit
points:
  1 model: 725
tags:
- unit
- unit/混沌骑士
- 混沌骑士
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 混沌[[factions/帝国骑士/units/acastus-knight-porphyrion.md\|巨人王型阿卡斯托斯骑士]] | 8" | 13 | 2+ | 30 | 6+ | 10 |

### 特殊保护
- 5+

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 阿卡斯托斯自动炮  | 48" | 2 | 3+ | 9 | -1 | 3 | — |
| 钢铁风暴导弹仓 | 48" | D6+6 | 3+ | 5 | 0 | 1 | [[core-rules/blast.md\|爆炸]]，[[core-rules/heavy.md\|重型]]，[[core-rules/indirect-fire.md\|曲射]] |
| 赫利俄斯防空导弹 | 48" | 3 | 3+ | 10 | -2 | D6 | [[core-rules/anti.md\|针对飞行2+]]，[[core-rules/heavy.md\|重型]] |
| 激光炮  | 48" | 1 | 3+ | 12 | -3 | D6+1 | — |
| 并联电磁激光炮 | 72" | D6 | 3+ | 18 | -4 | D6+6 | [[core-rules/blast.md\|爆炸]]，[[core-rules/twin-linked.md\|双联]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 装甲巨足 | 近战 | 6 | 4+ | 10 | -1 | 2 | — |

## 技能
- **阵营技能：恐惧使者**
- **阵营技能：超重型机甲**
- **火力堡垒**：如果此模型在移动阶段保持静止，直到本回合结束前，该模型装备的远程武器获得【致命一击】技能。
- **受损**：当该模型剩余1-10点耐伤，模型攻击的命中骰结果-1，OC属性-5。

## 单位构成
- **1个模型** — 725 分

## 关键词
- **阵营关键词**：Chaos Knights
- **普通关键词**：Vehicle，[[core-rules/walker.md|Walker]]，[[core-rules/titanic.md|Titanic]]，[[core-rules/towering.md|Towering]]，Knight Porphyrion，Acastus，Chaos