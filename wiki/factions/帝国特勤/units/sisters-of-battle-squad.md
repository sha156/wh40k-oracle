---
id: '000003818'
name_zh: 战斗修女小队
name_en: Sisters of Battle Squad
faction: 帝国特勤
type: unit
points:
  10 models (AGENTS OF THE IMPERIUM Detachment): 100
  10 models (Assigned Agent): 115
  10 models: 100
tags:
- unit
- unit/帝国特勤
- 帝国特勤
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 战斗修女小队 | 6" | 3 | 3+ | 1 | 7+ | 2 |

### 特殊保护
- 6+

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 精工风暴爆弹枪 | 24" | 2 | 3+ | 4 | 0 | 2 | [[core-rules/rapid-fire.md\|速射2]] |
| 爆弹手枪 | 12" | 1 | 3+ | 4 | 0 | 1 | [[core-rules/pistol.md\|pistol]] |
| 爆弹枪 | 24" | 1 | 3+ | 4 | 0 | 1 | [[core-rules/rapid-fire.md\|rapid fire 1]] |
| 复合武器 | 24" | 1 | 4+ | 4 | 0 | 1 | [[core-rules/anti.md\|ANTI-INFANTRY 4+]]，[[core-rules/devastating-wounds.md\|DEVASTATING WOUNDS]]，[[core-rules/rapid-fire.md\|RAPID FIRE 1]] |
| 降罪者爆弹枪 | 24" | 1 | 3+ | 4 | 0 | 1 | [[core-rules/anti.md\|ANTI-PSYKER 2+]]，[[core-rules/devastating-wounds.md\|DEVASTATING WOUNDS]]，[[core-rules/precision.md\|PRECISION]]，[[core-rules/rapid-fire.md\|RAPID FIRE 1]] |
| 重型爆弹枪 | 36" | 3 | 4+ | 5 | -1 | 2 | [[core-rules/heavy.md\|重型]]，[[core-rules/sustained-hits.md\|连击1]] |
| 地狱火手枪 | 6" | 1 | 3+ | 8 | -4 | D3 | [[core-rules/melta.md\|热熔2]]，[[core-rules/pistol.md\|手枪]] |
| 热熔枪 | 12" | 1 | 3+ | 9 | -4 | D6 | [[core-rules/melta.md\|热熔2]] |
| 教廷火焰喷射器 | 12" | D6 | N/A | 5 | 0 | 1 | [[core-rules/ignores-cover.md\|ignores cover]]，[[core-rules/torrent.md\|torrent]] |
| 教廷喷火手枪 | 12" | D6 | N/A | 4 | 0 | 1 | [[core-rules/ignores-cover.md\|IGNORES COvER]]，[[core-rules/pistol.md\|pISTOl]]，[[core-rules/torrent.md\|TORRENT]] |
| 教廷重型火焰喷射器 | 12" | D6 | N/A | 6 | -1 | 1 | [[core-rules/ignores-cover.md\|无视掩体]]，[[core-rules/torrent.md\|洪流]] |
| 多管热熔 | 18" | 2 | 4+ | 9 | -4 | D6 | [[core-rules/heavy.md\|重型]]，[[core-rules/melta.md\|热熔2]] |
| 等离子手枪-标准 | 12" | 1 | 3+ | 7 | -2 | 1 | [[core-rules/pistol.md\|手枪]] |
| 等离子手枪-过载 | 12" | 1 | 3+ | 8 | -3 | 2 | [[core-rules/pistol.md\|手枪]]，[[core-rules/hazardous.md\|危险]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 链锯剑 | 近战 | 3 | 4+ | 3 | -1 | 1 | — |
| 格斗武器 | 近战 | 1 | 4+ | 3 | -1 | 1 | — |
| 动力武器 | 近战 | 2 | 4+ | 4 | -2 | 1 | — |

## 技能
- **【阵营技能】：派遣特工**
- **信仰卫士**：当己方指挥阶段结束时，如果该模型位于由你控制的目标标记范围内，即使没有任何己方模型位于该目标标记范围内，该目标标记页将一直被你控制，直到该目标标记在任意回合开始或结束时被您的对手控制。
- **智天使**：每场战斗中您有一次机会，您可以花费0CP对该单位使用指挥重掷计谋，即使您在该阶段中已经对另一个单位使用过该计谋。
- **武器技能-帝国圣象**：持有者所在单位中模型的领导力属性提升1。

## 单位构成
- **10个模型 (AGENTS OF THE IMPERIUM Detachment)** — 100 分
- **10个模型 (Assigned Agent)** — 115 分
- **10个模型** — 100 分

## 关键词
- **阵营关键词**：Agents of the Imperium
- **普通关键词**：[[core-rules/infantry.md|Infantry]]，[[core-rules/grenades.md|Grenades]]，Imperium，Ordo Hereticus，Requisitioned，Sisters of Battle Squad