---
id: 000003816
name_zh: 死亡守望杀戮小队
name_en: Deathwatch Kill Team
faction: 帝国特勤
type: unit
points:
  5 models: 100
  10 models (<ky>AGENTS OF THE IMPERIUM</ky> Detachment): 190
  10 models (Assigned Agent): 200
  10 models: 190
tags:
- unit
- unit/帝国特勤
- 帝国特勤
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 死亡守望杀戮小队 | 6" | 4 | 3+ | 2 | 6+ | 2 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 爆弹枪 | 24" | 2 | 3+ | 4 | 0 | 1 | — |
| 破片炮 | 18" | D3 | 4+ | 7 | -1 | 2 | [[core-rules/blast.md\|爆炸]]，[[core-rules/heavy.md\|重型]]，速射D3 |
| 地狱火重型爆弹枪-重型爆弹枪 | 36" | 3 | 4+ | 5 | -1 | 2 | [[core-rules/heavy.md\|重型]]，[[core-rules/sustained-hits.md\|连击1]] |
| 地狱火重型爆弹枪-重型火焰喷射器 | 12" | D6 | N/A | 5 | -1 | 1 | [[core-rules/ignores-cover.md\|无视掩体]]，[[core-rules/torrent.md\|洪流]] |
| 复合武器 | 24" | 1 | 4+ | 4 | 0 | 1 | [[core-rules/anti.md\|针对步兵4+]]，[[core-rules/devastating-wounds.md\|毁灭伤害]]，[[core-rules/rapid-fire.md\|速射1]] |
| 追猎者爆弹枪 | 24" | 1 | 3+ | 4 | -1 | 2 | [[core-rules/heavy.md\|重型]]，[[core-rules/precision.md\|精准]] |
| 死亡守望霰弹枪 | 18" | 2 | 3+ | 4 | 0 | 2 | [[core-rules/assault.md\|突击]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 异形相位刃 | 近战 | 4 | 3+ | 5 | -2 | 1 | [[core-rules/devastating-wounds.md\|毁灭伤害]] |
| 格斗武器 | 近战 | 3 | 3+ | 4 | 0 | 1 | — |
| 死亡守望雷霆锤 | 近战 | 3 | 4+ | 10 | -2 | 3 | [[core-rules/devastating-wounds.md\|毁灭伤害]] |
| 动力武器 | 近战 | 3 | 3+ | 5 | -2 | 1 | — |
| 异形相位刃 | 近战 | 4 | 3+ | 5 | -2 | 1 | [[core-rules/devastating-wounds.md\|毁灭伤害]] |

## 技能
- **【阵营技能】：派遣特工**
- **诛杀异形**：当该单位中的模型进行攻击时，您可以重掷结果为1的命中掷骰。如果攻击的目标没有帝国或混沌关键词，则您可以重掷命中掷骰。
- **武器技能-阿斯塔特护盾**：持有者拥有4+无敌豁免

## 单位构成
- **5个模型** — 100 分
- **10个模型** — 190 分
- **10个模型** — 200 分
- **10个模型** — 190 分

## 关键词
- **阵营关键词**：Agents of the Imperium
- **普通关键词**：[[core-rules/infantry.md|Infantry]]，Kill Team，Deathwatch，Retinue，Ordo Xenos，Imperium，[[core-rules/grenades.md|Grenades]]，[[core-rules/battleline.md|Battleline]]