---
id: '000003820'
name_zh: 审判庭奇美拉装甲车
name_en: Inquisitorial Chimera
faction: 帝国特勤
type: unit
points:
  1 model: 60
tags:
- unit
- unit/帝国特勤
- 帝国特勤
version:
  points: MFM 2026-09-14T05:51:23.572407+00:00
  source: official-db
sources:
- book: https://assets.warhammer-community.com/eng_22-07_warhammer_40,000_faction_pack_imperia_agents-fttx9vrxuj-jug1nycmjn.pdf
  pages:
  - 10
updated: '2026-09-14'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 审判庭奇美拉装甲车 | 10" | 9 | 3+ | 11 | 7+ | 2 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 重型爆弹枪 | 36" | 3 | 4+ | 5 | -1 | 2 | [[core-rules/sustained-hits.md\|连击1]] |
| 重型火焰喷射器 | 12" | D6 | N/A | 5 | -1 | 1 | [[core-rules/ignores-cover.md\|无视掩体]]，[[core-rules/torrent.md\|洪流]] |
| 重机枪 | 36" | 3 | 4+ | 4 | 0 | 1 | [[core-rules/rapid-fire.md\|速射3]] |
| 猎杀导弹 | 48" | 1 | 4+ | 14 | -3 | D6 | [[core-rules/one-shot.md\|单发]] |
| 激光枪阵列 | 24" | 6 | 4+ | 3 | 0 | 1 | [[core-rules/rapid-fire.md\|速射6]] |
| 多管激光 | 36" | 4 | 4+ | 6 | 0 | 1 | — |
| 风暴爆弹枪 | 24" | 2 | 4+ | 4 | 0 | 1 | [[core-rules/rapid-fire.md\|速射2]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 装甲履带 | 近战 | 3 | 4+ | 6 | 0 | 1 | — |

## 技能
- **阵营：派遣特工**
- **迅速部署**：搭乘该运输工具的单位可以在突进后下车。这样做的单位视作在该阶段中进行了常规移动，并且不能在同一回合中进行冲锋，除此之外可以照常行动。

## 单位构成
- **1个模型** — 60 分

## 关键词
- **阵营关键词**：Agents of the Imperium
- **普通关键词**：[[core-rules/transport.md|Transport]]，[[core-rules/smoke.md|Smoke]]，Vehicle，Inquisitorial Chimera，Imperium，[[core-rules/dedicated-transport.md|Dedicated Transport]]，FRAME