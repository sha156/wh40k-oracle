---
id: 000000877
name_zh: 审判官
name_en: Inquisitor
faction: 帝国特勤
type: unit
points:
  1 model (AGENTS OF THE IMPERIUM Detachment): 55
  1 model (Assigned Agent): 65
  1 model: 55
tags:
- unit
- unit/帝国特勤
- 帝国特勤
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 执政官 | 6" | 3 | 4+ | 4 | 6+ | 1 |

### 特殊保护
- 5+

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 爆弹手枪 | 12" | 1 | 3+ | 4 | 0 | 1 | [[core-rules/pistol.md\|手枪]] |
| 复合武器 | 24" | 1 | 3+ | 4 | -2 | 1 | [[core-rules/anti.md\|针对步兵4+]]，[[core-rules/devastating-wounds.md\|毁灭伤害]]，[[core-rules/rapid-fire.md\|速射1]] |
| 灵能冲击波 | 18" | 2D6 | N/A | 3 | -2 | 1 | [[core-rules/devastating-wounds.md\|毁灭伤害]]，灵能，[[core-rules/torrent.md\|洪流]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 审判庭近战武器 | 近战 | 5 | 3+ | 4 | -2 | 1 | — |
| 灵能武器 | 近战 | 4 | 3+ | 5 | -2 | D3 | 灵能 |

## 技能
- **【阵营技能】：派遣特工**
- **审判庭权力**：当该模型领导一个联合单位时，可以搭乘任何其领导的护卫单位可以搭乘的运输工具。
- **审判庭权威**：当对该模型所在的单位使用一个计谋时掷一枚D6，如果掷骰结果为3+，您获得1CP。
- **武器技能-受祝守护**：当持有者领导一个联合单位时，该单位中的模型拥有6无敌豁免。
- **武器技能-灵能赠礼**：持有者拥有灵能者关键词。

## 单位构成
- **1个模型 (AGENTS OF THE IMPERIUM Detachment)** — 55 分
- **1个模型 (Assigned Agent)** — 65 分
- **1个模型** — 55 分

## 关键词
- **阵营关键词**：Agents of the Imperium
- **普通关键词**：Inquisitor，Imperium，[[core-rules/infantry.md|Infantry]]，[[core-rules/character.md|Character]]，[[core-rules/grenades.md|Grenades]]