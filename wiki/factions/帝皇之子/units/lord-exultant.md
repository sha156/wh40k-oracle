---
id: 000004078
name_zh: 极乐领主
name_en: Lord Exultant
faction: 帝皇之子
type: unit
points:
  1 model: 80
tags:
- unit
- unit/帝皇之子
- 帝皇之子
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 极乐领主 | 7" | 4 | 3+ | 5 | 6+ | 1 |

### 特殊保护
- 4+

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 爆弹手枪 | 12" | 1 | 2+ | 4 | 0 | 1 | [[core-rules/pistol.md\|手枪]] |
| 等离子手枪（普通） | 12" | 1 | 2+ | 7 | -2 | 1 | [[core-rules/pistol.md\|手枪]] |
| 等离子手枪（过载） | 12" | 1 | 2+ | 8 | -3 | 2 | [[core-rules/pistol.md\|手枪]]，[[core-rules/hazardous.md\|危险]] |
| 音波手枪 | 12" | 3 | 2+ | 5 | -1 | 2 | [[core-rules/pistol.md\|手枪]]，[[core-rules/ignores-cover.md\|忽视掩体]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 格斗武器 | 近战 | 6 | 2+ | 4 | 0 | 1 | — |
| 精工动力剑 | 近战 | 5 | 2+ | 5 | -2 | 2 | [[core-rules/precision.md\|精准]] |
| 凤凰动力矛 | 近战 | 5 | 2+ | 7 | -2 | 2 | [[core-rules/lance.md\|迅猛冲锋]] |
| 动力拳 | 近战 | 5 | 2+ | 8 | -2 | 2 | — |
| 狂喜之鞭 | 近战 | 4 | 2+ | 4 | -1 | 1 | [[core-rules/extra-attacks.md\|额外攻击]] |

## 技能
- **【阵营技能】：嗜欲恶徒**
- **完美主义者**：当本模型领导一个单位时，那个单位中模型装备的武器拥有【致命一击】。
- **狂笑突袭**：每场游戏仅一次，在任意近战阶段开始时可以使用本能力，如果你这么做，本模型装备的近战武器 A 值增加 3，AP 值增强 1 点。
- **战帮之主**：如果本模型在宣布战斗阵型步骤时加入 一个帝皇之子战线单位，则获得【斥候 6】与【渗 透】。

## 单位构成
- **1个模型** — 80 分

## 关键词
- **阵营关键词**：Emperor’s Children
- **普通关键词**：[[core-rules/character.md|Character]]，Slaanesh，Lord Exultant，[[core-rules/infantry.md|Infantry]]，Chaos，[[core-rules/grenades.md|Grenades]]