---
id: 000004079
name_zh: 施虐者
name_en: Tormentors
faction: 帝皇之子
type: unit
points:
  5 models: 80
  10 models: 160
tags:
- unit
- unit/帝皇之子
- 帝皇之子
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 施虐者 | 7" | 4 | 3+ | 2 | 6+ | 2 |

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 爆弹手枪 | 12" | 1 | 3+ | 4 | 0 | 1 | [[core-rules/pistol.md\|手枪]]，[[core-rules/precision.md\|精准]] |
| 爆弹枪 | 24" | 2 | 3+ | 4 | 0 | 1 | [[core-rules/precision.md\|精准]] |
| 热熔枪 | 12" | 1 | 3+ | 9 | -4 | D6 | [[core-rules/melta.md\|热熔2]]，[[core-rules/precision.md\|精准]] |
| Plasma gun - standard | 24" | 1 | 3+ | 7 | -2 | 1 | precision，[[core-rules/rapid-fire.md\|rapid fire 1]] |
| Plasma gun - supercharge | 24" | 1 | 3+ | 8 | -3 | 2 | hazardous，precision，[[core-rules/rapid-fire.md\|rapid fire 1]] |
| Plasma pistol - standard | 12" | 1 | 3+ | 7 | -2 | 1 | pistol，precision |
| Plasma pistol - supercharge | 12" | 1 | 3+ | 8 | -3 | 2 | hazardous，pistol，precision |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 格斗武器 | 近战 | 3 | 3+ | 4 | 0 | 1 | — |
| 动力剑 | 近战 | 4 | 3+ | 5 | -2 | 1 | — |
| 狂喜之鞭 | 近战 | 6 | 3+ | 4 | -1 | 1 | — |

## 技能
- **【阵营技能】：嗜欲恶徒**
- **亵渎目标**：你的指挥阶段结束时，如果本单位位于一个你控制的任务目标点范围内，那么直到你的对手在任意阶段结束时占领该目标点之前，即使该目标点上没有你的单位，该目标点仍然由你控制。
- **装备技能-纵欲圣印**：在你的射击阶段或任意近战阶段结束时，如果装备者所在的单位在本阶段中消灭了一个或多个敌方单位，则你为本单位进行一次领导力测试，如果测试成功，你获得一点 CP。

## 单位构成
- **5个模型** — 80 分
- **10个模型** — 160 分

## 关键词
- **阵营关键词**：Emperor’s Children
- **普通关键词**：Chaos，Slaanesh，Tormentors，[[core-rules/battleline.md|Battleline]]，[[core-rules/infantry.md|Infantry]]，[[core-rules/grenades.md|Grenades]]