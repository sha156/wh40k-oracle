---
id: '000001482'
name_zh: 战刃侍从
name_en: Armiger Warglaive
faction: 帝国骑士
type: unit
points:
  1 model: 140
tags:
- unit
- unit/帝国骑士
- 帝国骑士
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| 战刃侍从 | 12" | 9 | 3+ | 14 | 7+ | 6 |

### 特殊保护
- 5+

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 热熔枪 | 12" | 1 | 3+ | 9 | -4 | D6 | [[core-rules/melta.md\|热熔 2]] |
| 封臣重机枪 | 36" | 3 | 3+ | 4 | -1 | 1 | [[core-rules/rapid-fire.md\|速射 3]] |
| 热能矛 | 18" | 2 | 3+ | 12 | -4 | D6 | [[core-rules/melta.md\|热熔 4]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 链锯阔刃-重击 | 近战 | 4 | 3+ | 10 | -3 | 3 | — |
| 链锯阔刃-横扫 | 近战 | 8 | 3+ | 8 | -2 | 1 | — |

## 技能
- **先锋荣耀**：每当本模型完成冲锋移动后，直到当前回合结束前，本模型装备的链锯阔刃-重击 A 值+1，链锯阔刃-横扫  A 值+2。
- **骑士誓言**

## 单位构成
- **1个模型** — 140 分

## 关键词
- **阵营关键词**：Imperial Knights
- **普通关键词**：Warglaive，Armiger，Imperium，[[core-rules/walker.md|Walker]]，Vehicle