---
id: '000008965002'
name_en: DETONATOR
faction: 混沌星际战士
type: stratagem
detachment: Deceptors
cp: 1
phase: Any phase
stratagem_type: Deceptors – Wargear Stratagem
tags:
- stratagem
- stratagem/混沌星际战士
- 混沌星际战士
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11 版镜像）
updated: '2026-07-26'
---

1 CP、Any phase、[[factions/混沌星际战士/detachments/deceptors.md|Deceptors]] 分队、Wargear Stratagem。

## 使用时机

Any phase, just after an enemy model with the Deadly Demise ability (excluding [[core-rules/titanic.md|TITANIC]] models) is destroyed

## 使用对象

One HERETIC ASTARTES CHARACTER unit from your army that was within 18" of that enemy model when it was destroyed.

## 效果

Your opponent does not roll to determine whether mortal wounds are inflicted by their model’s Deadly Demise ability. Instead, mortal wounds are automatically inflicted (if that ability inflicts a random number of mortal wounds, your opponent rolls to determine that number as normal).