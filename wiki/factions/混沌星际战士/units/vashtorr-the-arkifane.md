---
id: '000002679'
name_zh: 瓦什托尔
name_en: Vashtorr The Arkifane
faction: 混沌星际战士
type: unit
points:
  1 model: 220
tags:
- unit
- unit/混沌星际战士
- 混沌星际战士
version:
  points: MFM 2026-07-23 14:17
  source: official-db
sources:
- book: 官方结构库 db/wh40k.sqlite（Wahapedia 11版镜像 + MFM 官方点数）
updated: '2026-07-23'
---

## 属性表
| 模型 | M | T | SV | W | LD | OC |
|---|---|---|---|---|---|---|
| Vashtorr The Arkifane | 12" | 10 | 2+ | 14 | 6+ | 3 |

### 特殊保护
- 4+

## 射击武器
| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 魔爪喷射 | 12" | D6 | N/A | 5 | -2 | 1 | [[core-rules/anti.md\|anti-vehicle 4+]]，[[core-rules/torrent.md\|torrent]] |

## 近战武器
| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|---|---|---|---|---|---|---|---|
| 瓦什托尔魔锤（重击） | 近战 | 6 | 2+ | 14 | -3 | 3 | [[core-rules/anti.md\|反载具 4+]]，[[core-rules/devastating-wounds.md\|毁灭伤害]] |
| 瓦什托尔魔锤（横扫） | 近战 | 12 | 2+ | 8 | -1 | 2 | [[core-rules/anti.md\|反载具 4+]]，[[core-rules/devastating-wounds.md\|毁灭伤害]] |

## 技能
- **【阵营技能】：黑暗契约**
- **不洁机魂【光环】**：本模型 6 寸内的恶魔载具友军单位所 装备的武器 S+2
- **重组现实**：本模型 18 寸内的敌方单位攻击本模型时，命中掷骰结果-1，且所装备的射击武器在本阶段中获得【危险】技能
- **魔机契约**：本模型位于一个或更多己方恶魔载具单位3寸范围内时，本模型拥有【独行特工】技能
- **严重损伤**：当本模型 W 值为 1-4 时，进行的攻击命中结果-1

## 单位构成
- **1个模型** — 220 分

## 关键词
- **阵营关键词**：Heretic Astartes
- **普通关键词**：Fly，Chaos，Vashtorr the Arkifane，[[core-rules/epic-hero.md|Epic Hero]]，Daemon，Monster，[[core-rules/character.md|Character]]