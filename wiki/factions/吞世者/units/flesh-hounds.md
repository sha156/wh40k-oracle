---
id: 000004108
name_en: Flesh Hounds
faction: 吞世者
type: unit
tags:
- unit
- unit/吞世者
- 吞世者
sources:
- book: Faction Pack Chaos Daemons
  pages:
  - 32
  - 33
raw:
- data_refined\Faction Pack Chaos Daemons\page_032.md
- data_refined\Faction Pack Chaos Daemons\page_033.md
---

## 属性表

| 模型 | M | T | SV | W | LD | OC |
|------|----|----|------|-----|------|------|
| 血肉猎犬 | 12" | 4 | 7+ | 2 | 7+ | 1 |

## 远程武器

| 武器 | 射程 | A | BS | S | AP | D | 技能 |
|------|------|---|-----|-----|-----|-----|------|
| 燃烧咆哮 (Burning roar) | 12" | D6 | N/A | 4 | 0 | 1 | [[core-rules/ignores-cover.md|无视掩体]]，[[core-rules/torrent.md|洪流]] |

## 近战武器

| 武器 | 射程 | A | WS | S | AP | D | 技能 |
|------|------|---|-----|-----|-----|-----|------|
| 浸血之牙 (Gore-drenched fangs) | 近战 | 3 | 3+ | 5 | -1 | 1 | - |

## 技能

### 核心技能
- **深入打击 (Deep Strike)**：单位可在[[core-rules/deployment.md|部署阶段]]通过[[core-rules/deep-strike.md|深入打击]]能力部署。

### 阵营技能
- **混沌之影 (The Shadow of Chaos)**：此单位的[[混沌统帅光环]]和[[暗影之步]]能力适用[[混沌之影]]规则。

### 单位技能
- **亚空间猎手 (Hunters from the Warp)**：在对手[[core-rules/battle-round.md|回合]]结束时，若此单位未与任何敌方单位处于[[core-rules/engagement-range.md|交战范围]]内，你可以将其从战场上移除并置入[[core-rules/strategic-reserves.md|战略预备队]]。

### 装备技能
- **恐虐项圈 (Collar of Khorne)**：携带者在对抗[[core-rules/psychic-attacks.md|灵能攻击]]时获得[[core-rules/feel-no-pain.md|不怕疼 3+]]能力。
- **无敌豁免 (Invulnerable Save)**：5+

## 单位构成
- **1 只血肉猎犬头领 (Gore Hound)**
- **4-9 只血肉猎犬 (Flesh Hounds)**

头领装备：燃烧咆哮、浸血之牙、恐虐项圈。
每只血肉猎犬装备：浸血之牙、恐虐项圈。

**装备选项**：无

## 关键词
- **阵营关键词**：Legiones Daemonica
- **普通关键词**：野兽，混沌，恶魔，恐虐，血肉猎犬