...
- 吞世者
sources:
- book: Faction Pack Death Guard
  pages:
  - 5
raw:
- data_refined\Faction Pack Death Guard\page_005.md
verify_warn: true
---
...
## 技能
- **Deadly Demise D6（核心）**
- **Nurgle's Gift (Aura)（阵营）**
- **Scuttling Walker**：每次该单位进行普通移动、挺进或撤退时，它可以穿过模型（泰坦单位除外）和地形特征。这样做时，它可以进入敌方模型的交战范围，但不能在该移动结束时处于其交战范围内，并且任何绝望脱逃测试自动通过。
- **Barrage of Filth**：在你的射击阶段，该模型射击后，选择一个被其中一次或多次攻击击中的敌方单位。直到该阶段结束，该单位不能获得 [[core-rules/benefit-of-cover.md|掩体优势]]。
- **Invulnerable Save**：5+

## 单位构成
- **模型数量**：1 个亵渎者
- **装备选项**：本模型可以携带上述任意远程武器组合，并可从电鞭与剪切爪中选择一组近战武器。

## 关键词
- **阵营关键词**：Death Guard
- **普通关键词**：Vehicle, Walker, Chaos, Nurgle, Daemon, Defiler